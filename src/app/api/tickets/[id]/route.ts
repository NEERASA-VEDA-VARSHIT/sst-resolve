 ...
 *   - Auth: Required
 *   - Access Control: Student (own), Admin (assigned+viewable), Super Admin (all), Committee (tagged)
 *   - Returns: Full ticket details with creator, assignee, category info
 * 
 * PATCH → Update Ticket
 *   - Auth: Required (Admin+)
 *   - Update description, location, priority, metadata
 *   - Returns: 200 OK with updated ticket
 * 
 * DELETE → Hard Delete Ticket
 *   - Auth: Required (Super Admin only)
 *   - Use sparingly - removes ticket permanently
...
export async function GET(
  ...
  try {
    ...
    // Admin/SPOC → only assigned tickets
    if (role === "admin") {
      const [userRow] = await db
        .select({ id: users.id })
        .from(users)
      ...
      if (!staffRow || ticketRecord.assigned_to !== staffRow.id) {
        return NextResponse.json({ error: "Forbidden" }, { status: 403 });
      }
    }
    ...
    return NextResponse.json(ticketRecord, { status: 200 });
...
export async function PATCH(
  ...
  try {
    ...
    const allowed = ["description", "location", "metadata"];
...
// import { db, tickets, ticket_committee_tags, committee_members, staff, categories, users } from "@/db";
...
// import { normalizeStatusForComparison } from "@/utils";
...
//         userNumber: tickets.user_number,
...
//         const isClosedOrResolved = ticket.status === "RESOLVED";
...
//             : user.emailAddresses[0]?.emailAddress || "Committee Member";
//         } catch (e) {
...
//       const updateData: any = { status: enumStatus, updated_at: new Date() };
...
//       if (isAdmin) {
//         // Get admin's staff ID for assignment
//         const dbUser = await getOrCreateUser(userId);
...
//       const wasClosedOrResolved = ticket.status === "RESOLVED";
...
//             .from(users)
//             .where(eq(users.id, updatedTicket.created_by))
//             .limit(1);

//           const studentEmail = creator?.email || null;
//           if (studentEmail) {
