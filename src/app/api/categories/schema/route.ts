...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch category - explicitly select columns to avoid Drizzle issues with new columns
    let category;
    if (categoryId) {
      const [cat] = await db
        .select({
    ...
    // Fetch subcategories - explicitly select columns to avoid Drizzle issues with new columns
    ...
    // Fetch fields for each subcategory
